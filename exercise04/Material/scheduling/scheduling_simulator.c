...
void initiate_all_processes() {
	...
	int a, b, c;
	while (1) {
		int assigned = fscanf(ptr, "%d %d %d ", &a, &b, &c);
		if (assigned < 0) {
			return;
		}
...
void perform_scheduling() {
	...
	if ((running_process->status == STATUS_BLOCKED)
			| (running_process->status == STATUS_READY)) {
		select_next_process();
	}
}
...
process* create_process(parameters params) {
	...
	set_ready(&ready_list, new_process);
	return new_process;
}
